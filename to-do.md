...
| ID | Pri | Status | Blocks | Description | Notes |
|----|-----|--------|--------|-------------|-------|
| T-243 | P2 | PARKED (E, GHA 23582386358) | — | **Merge `feature/hot-loop-opt` to `cpp-search`.** FlatBlock struct, flat EW indirect functions, TBR prefetch. Confirmed 1.4% speedup at 180 tips on Hamilton (median 11.538→11.360s, p=0.001, n=10). | PR #230 open. GHA failed: flaky parallel timeout test (test-ts-parallel.R:85, ARM64). Fix on cpp-search (371270b3) — needs merge into feature/hot-loop-opt. TS-HotLoop worktree exists. |
...
| ID | Pri | Status | Blocks | Description | Notes |
|----|-----|--------|--------|-------------|-------|
| T-255 | P2 | PARKED (E, GHA 23590370573) | — | **Reduce drift in default and thorough presets.** T-254 confirmed drift has zero score, MPT, or diversity benefit and costs 10–22% of replicates. Set `driftCycles=0` in default; reduce from 12 to 0 in thorough. Reallocate saved time to extra replicates. | T-251: drift is 30–170× less efficient (ms/step) than next-worst phase. T-254: no MPT diversity benefit on 3 datasets × 3 seeds × 30s. |
