...
| ID | Pri | Status | Blocks | Description | Notes |
|----|-----|--------|--------|-------------|-------|
| T-254 | P2 | OPEN | — | **Drift MPT diversity experiment.** Compare pool size and MPT count with `driftCycles=0` vs `driftCycles=2` on Geisler2001, Zhu2013, Wortley2006 (3 seeds, 30s+120s). Drift produces equal-score outcomes 90–95% of the time (T-251); question is whether those displace to topologically distinct basins that help MPT enumeration, or just waste time returning to the same optimum. | Validates whether drift can be safely reduced. Benchmark script: `bench_trajectory.R` has dataset prep; extend with pool/MPT comparison. |
| T-255 | P2 | OPEN | T-254 | **Reduce drift in default preset.** If T-254 confirms drift has negligible MPT diversity benefit: set `driftCycles=0` in default and sprint presets; reduce to 1 in thorough. Reallocate saved time (~20% of wall clock) to extra ratchet or sectorial rounds. | T-251: drift is 30–170× less efficient (ms/step) than next-worst phase. Currently 16–23% of wall time, <1% of score improvement. |
| T-256 | P2 | OPEN | — | **Increase sectorial search intensity.** TNT dedicates ~67% of intra-replicate time to sectorial search; TreeSearch's XSS+RSS+CSS takes 6–10% of time but has respectable efficiency (5–20 ms/step). Experiment: double `xssRounds` and `rssRounds`; benchmark on gap datasets (3 seeds, 30s). | Independent of drift experiment. Check for diminishing returns — sectorial search may plateau after a few rounds if sector sizes are too small. |
