...
| ID | Pri | Status | Blocks | Description | Notes |
|----|-----|--------|--------|-------------|-------|
| T-261 | P2 | PARKED (E, GHA 23597418929) | — | **Eliminate `std::fill` zeroing in `reset_states()`.** `full_rescore()` calls `reset_states()` which zeroes all 5 state arrays (`prelim`, `final_`, `down2`, `subtree_actives`, `local_cost`) before scoring. VTune shows this costs **3.9% of TBR time** (~1.2s/30s). The Fitch downpass overwrites all internal-node `prelim` entries bottom-up; the uppass overwrites `final_`; NA pass 1 overwrites `subtree_actives`; NA pass 3 overwrites `down2`. The zeroing is likely redundant. **Task:** (1) Audit each scoring pass in `ts_fitch_na.h` to confirm every internal node entry is written before read. Pay attention to: root node (`final_[root] = prelim[root]` at pass 2 start), tip `down2`/`subtree_actives` updates in pass 2 (lines ~197-204), and `local_cost` accumulation in the downpass. (2) If confirmed safe, replace `reset_states()` body with just `load_tip_states(ds)`. (3) Run full test suite to catch any scoring regressions. | Low effort, high confidence. `reset_states` is in `ts_tree.cpp:267`. EW-only scoring (no NA arrays) is the simplest case — confirm that first, then extend to NA. Feature branch `feature/eliminate-fill`. |
| T-262 | P2 | PARKED (E, GHA 23597418929) | — | **Reduce `load_tip_states` scope in `full_rescore`.** `load_tip_states()` copies tip data from `ds.tip_states` into `prelim`, `final_`, and `subtree_actives` for all tips before every `score_tree()`. VTune: **5.2% of TBR time** (1.6s/30s). Not all arrays need reloading every time: `final_` tip entries are overwritten by the uppass (pass 2, line ~197: `D2 = final for tips`), and `subtree_actives` tips are updated by the tip uppass (pass 2, lines ~201-204). Only `prelim` tip entries (observed states) are used as-is by the downpass. **Task:** (1) Audit which tip arrays are actually read before being written during scoring. (2) If `final_` and `subtree_actives` tips are overwritten by the uppass before being read, remove those copies from `load_tip_states()` and only reload `prelim`. (3) Test carefully — the NA tip update in pass 2 reads `final_[parent]` which may depend on tip `final_` being set from `prelim`. | Low-medium effort. Can be done on same branch as T-261. Savings depend on audit results — may find some tip arrays DO need initialization. |
| T-263 | P2 | PARKED (F, GHA 23596902964) | — | **Selective StateSnapshot save/restore in TBR.** `StateSnapshot::save()` and `::restore()` in `ts_tbr.cpp:216-246` memcpy the **entire** prelim, final_, down2, subtree_actives, local_cost, and postorder arrays before each candidate evaluation and restore on rejection. VTune: **14.6% of TBR time** (4.53s/30s). At 88 tips, each save/restore copies ~190 KB; at 180 tips, ~380+ KB. Most of this data is untouched — only nodes on the path from clip/regraft points to root are modified by `apply_tbr_move()` + `full_rescore()`. **Task:** (1) In `apply_tbr_move()`, track which node indices are modified (the clip subtree, the regraft path, and their ancestors to root). (2) Replace bulk memcpy with selective save/restore of only dirty node entries. (3) For `postorder`, either save/rebuild (cheap at ~175 ints) or track whether it changed. (4) Benchmark: must show measurable wall-time improvement on Dikow2009 (88t) and ideally mbank_X30754 (180t). **Alternative approach:** eliminate the snapshot entirely — after rejection, call `full_rescore()` to recompute from scratch. This trades snapshot cost (14.6%) for an extra `full_rescore` on rejection (~29% × rejection_rate). Only viable if T-261+T-262 make `full_rescore` very cheap. Benchmark both approaches. | Medium-high effort. Most impactful single change (~10–12% savings). Needs careful correctness testing — any node missed in the dirty set produces silent score corruption. Feature branch `feature/selective-snapshot`. |
