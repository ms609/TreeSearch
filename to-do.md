...
| ID | Pri | Status | Blocks | Description | Notes |
|----|-----|--------|--------|-------------|-------|
| T-255 | P2 | OPEN | — | **Reduce drift in default and thorough presets.** T-254 confirmed drift has zero score, MPT, or diversity benefit and costs 10–22% of replicates. Set `driftCycles=0` in default; reduce from 12 to 0 in thorough. Reallocate saved time to extra replicates. | T-251: drift is 30–170× less efficient (ms/step) than next-worst phase. T-254: no MPT diversity benefit on 3 datasets × 3 seeds × 30s. |
