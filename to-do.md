...
| T-208 | P2 | DONE | — | **[Bug] `random_topology_tree` ignores constraints.** When `adaptiveStart=true` AND constraints active, bandit selects RANDOM_TREE → constraint violation. | Fix cherry-picked to cpp-search (24427c9a). PR #219 closed. |
...
| T-211 | P2 | CLOSED | — | **[Bug] Stale `final_` in temper candidate scoring.** Conservative-only impact; not worth fixing. | Closed by C (round 14): stale final_ affects Boltzmann screening only; `temper_full_rescore` gates accepted moves. |
...
| ID | Pri | Status | Blocks | Description | Notes |
|----|-----|--------|--------|-------------|-------|
| T-213 | P2 | PR #223 (A) | — | **Implement `impose_constraint()` for post-hoc topology repair.** Enables NNI perturbation + fuse under constraints. | Branch `feature/impose-constraint`, worktree `TS-ImpCon`. GHA 23528483696. |
