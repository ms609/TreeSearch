...
| ID | Pri | Status | Blocks | Description | Notes |
|----|-----|--------|--------|-------------|-------|
| S-RED | dyn | OPEN | — | **Standing: Red-team review** | Last run: 2026-03-27 focus 23 by F (ts_fitch.cpp 844+288 lines + ts_simd.h 379 lines). No bugs found. Downpass/uppass, incremental, indirect length, NA-aware flat variants, IW/profile delta precomputation all correct. `any_hit_reduce3` = OR(clip & (A|B)) verified. Empty `if (!root_changed)` in fitch_incremental_uppass is harmless dead code (clip_ancestor forced dirty). Next: ts_fitch_na.h + ts_fitch_na_incr.h (NA three-pass + incremental). |
