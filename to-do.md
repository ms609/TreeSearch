...
| T-305 | P3 | OPEN | — | **T-302 qmApp fix lacks a positive-path regression test.** `test-PolEscapa.R` new assertions in d24cd7af only test the contrast-row error path; the pre-existing `wiwaxia` test was already passing pre-fix. The qmApp scalar-unwrap itself is not covered. Recommend synthesizing a phyDat with ≥2 all-applicable contrast rows + a tip with both inapp and app tokens; assert `LengthAdded` returns non-negative deltas matching independent computation. Also add a parallel test for the analogous `qm` fix applied 2026-05-26 (see commit on cpp-search). | Found by /red-team areas 8+9 (2026-05-26). [m:sonnet e:medium] |
