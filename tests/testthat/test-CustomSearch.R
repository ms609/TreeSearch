...
test_that("Tree can be found", {
  ...
  mp1 <- RootTree(
    suppressWarnings(Morphy(phy11, tree = CollapseNode(random11, 13),
           ratchIter = 1))[[1]],
    "a")
  expect_true(all.equal(mp1, comb11))
  expect_true(all.equal(
    suppressWarnings(Morphy(phy11, tree = random11, verbosity = 0L))[[1]],
    comb11
  ))
  expect_true(all.equal(
    suppressWarnings(Morphy(phy11, random11, ratchIter = 0, verbosity = 0L))[[1]],
    comb11
  ))
  ...
  iw <- suppressWarnings(
    Morphy(phy11, random11, ratchIter = 1, tbrIter = 5,
...
test_that("Tree search finds shortest tree", {
  ...
  on.exit(suppressWarnings(UnloadMorphy(morphyObj)))
...
test_that("Profile parsimony works in tree search", {
  ...
  quickTS <- TreeSearch(rTree, dataset, EdgeSwapper = RootedNNISwap, 
                        maxIter = 1600, maxHits = 40, verbosity = 0)
  expect_equal(42, attr(quickTS, "score"))
  
  quickFitch <- Ratchet(rTree, dataset, suboptimal = 2, 
