  ...
  for (int clip_node = 0; clip_node < tree.n_node; ++clip_node) {
    ...
    for (int ri = 0; ri < n_reroot && !found; ++ri) {
      ...
      for (int below = 0; below < tree.n_node; ++below) {
        ...
        if (na_dir_audit) {
          double s_dir = nadir::candidate_score(ds, na_dir_cf, rp, rc, below);
          if (std::fabs(s_dir - s) > na_dir_tol) {
            Rcpp::stop("TS_NA_DIR_AUDIT mismatch: directional=%.4f full_rescore=%.4f "
                       "(clip=%d rp=%d rc=%d below=%d)",
                       s_dir, s, clip_node, rp, rc, below);
          }
        }
