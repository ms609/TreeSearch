...
PTResult parallel_temper_search(
  ...
  std::vector<int> pair_attempts(nc - 1, 0);
  std::vector<int> pair_accepts(nc - 1, 0);

  for (int round = 0; round < params.rounds; ++round) {

    double cold_before_round = scores[0];

    // Cold chain: TBR to convergence
    {
      ...
      if (diag) {
        ...
        diag->cold_tbr_total_ms += ms;
        ChainRoundDiag crd;
        crd.round = round; crd.chain_idx = 0; crd.temperature = 0.0;
        crd.score_before = cold_before_round; crd.score_after = scores[0];
        crd.n_accepted = 0; crd.n_improved = 0; crd.n_attempted = 0;
        crd.elapsed_ms = ms;
        diag->chain_log.push_back(crd);
      }
    }
    ...
    for (int i = 1; i < nc; ++i) {
      ...
      TemperResult tr = stochastic_tbr_phase(chains[i], ds, tp, cd,
                                              check_timeout);
      ...
      if (diag) {
        auto t1 = std::chrono::steady_clock::now();
        double ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
        diag->hot_stochastic_total_ms += ms;
        ChainRoundDiag crd;
        crd.round = round; crd.chain_idx = i; crd.temperature = temps[i];
        crd.score_before = sb; crd.score_after = scores[i];
        crd.n_accepted = tr.n_accepted; crd.n_improved = tr.n_improved;
        crd.n_attempted = tr.n_attempted;
        crd.elapsed_ms = ms;
        diag->chain_log.push_back(crd);
      }
    }
    ...
    if (diag) {
      if (scores[0] < cold_before_swaps - 1e-10)
        ++diag->cold_improvements_from_swaps;
      diag->cold_scores.push_back(scores[0]);
    }

    if (pool && scores[0] < pool_best) pool_best = scores[0];
    for (int i = 0; i < nc; ++i)
  ...
  if (diag) {
    diag->pair_acceptance_rates.resize(nc - 1);
    for (int i = 0; i < nc - 1; ++i)
      diag->pair_acceptance_rates[i] = pair_attempts[i] > 0
          ? static_cast<double>(pair_accepts[i]) / pair_attempts[i] : 0.0;
    auto pt_end = std::chrono::steady_clock::now();
    diag->total_pt_ms =
        std::chrono::duration<double, std::milli>(pt_end - pt_start).count();
  }
