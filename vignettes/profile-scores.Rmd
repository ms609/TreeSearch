...
justificaion for a parsimony approach, which this brief overview will not do justice to;
...
which that trait evolved twice.
...
in this case, assumptions of homoplasy.
...
Now consider two competing explanations for an event: one, that three
consecutive rolls of two dice each produced a seven; two, that two consecutive
rolls of two dice each produced a twelve.  The former event corresponds to 
$3 \times 2.58 = 7.75\textrm{ bits}$ of information, so is less surprising
than the latter, which represents $2 \times 5.16 = 10.34\textrm{ bits}$,
...
If we learn that the character contains exactly one step on the true tree,
then we learn that the true tree is one of the fifteen six-leaf trees on which
a single step can be reconstructed.  There are 105 labelled six-leaf trees,
so we have gained $-log_2{15 / 105} = 2.81\textrm{ bits}$ of information.
If we learn that the character contains exactly two steps, this

Nine of the 105 labelled six-leaf trees are consistent with this character
exhibiting a single step (`Carter1(1, 3, 3)`), so learning that the character
contains exactly one step yields $-log_2\frac{9}{105} = 3.54\textrm{ bits}$.
A second step is consistent with 63 trees, so learning that the character
contains exactly two steps yields $-log_2\frac{63}{105} = 0.74\textrm{ bits}$.
More than two steps are consistent with all 105 six-leaf trees, so provide
no information.
