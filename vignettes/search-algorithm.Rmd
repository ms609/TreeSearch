...
are protected from the final collapse pass, whichever branch happens to realise
them.
...
output stage has since canonicalised.
...
`TreeLength()` canonicalises identically, so a reported x-transformation score
is the length of the tree in hand, and one topology has one length.
This affects reporting only: the quantity the search optimises is unchanged.
