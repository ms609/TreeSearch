...
- **X-transformation** [@Goloboff2021b]: recodes hierarchically-dependent
...
replicates across six strategy arms:

| Arm | Type | Description |
|-----|------|-------------|
| Wagner-random | Fresh | Random addition-order Wagner tree |
| Wagner-Goloboff | Fresh | Goloboff (2014) biased addition order |
| Wagner-entropy | Fresh | Entropy-biased addition order |
| Random tree | Fresh | Purely random topology (no character data) |
| Pool-ratchet | Pool | Ratchet perturbation from a pool tree |
| Pool-NNI | Pool | NNI perturbation from a pool tree |
...
**Pool-based arms** are only available once the pool contains at least two
best-score trees, ensuring meaningful diversity for perturbation.
When unavailable, they are excluded from the sampling step.
...
7. **Tree drifting**
8. **Final TBR** polish
9. **Pool insertion** + tree fusing (every `fuseInterval` replicates)
...
All presets enable NNI warmup (`nniFirst = TRUE`) and consensus-stability
stopping (`consensusStableReps = 3` or 2 for `large`).
