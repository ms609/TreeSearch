...
> **Note:** For a standard implied weights analysis, use
> `MaximizeParsimony(dataset, concavity = k)`, which is faster and uses the
> full driven-search pipeline (see the
> [tree search vignette](tree-search.html#implied-weighting)).
> The example below instead shows how you would implement a scorer of
> comparable complexity yourself, entirely in R, using the custom-search
> framework -- useful as a template for optimality criteria that TreeSearch
...
theoretical minimum.  We can compute both quantities from a plain `phyDat`
object using existing `TreeSearch` functions -- no data-preparation hook is
needed, as we simply compute the minimum lengths once, up front, and let our
`TreeScorer` close over the result:
