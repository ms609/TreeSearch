...
It automatically selects how hard to search, based on dataset size; the
`effort` argument shifts that choice up or down
...
plot(TreeTools::Consensus(bestTrees))
...
```{r plot-label-nodes}
par(mar = rep(0.25, 4), cex = 0.75) # make plot easier to read
majCons <- TreeTools::Consensus(bestTrees, p = 0.5)
...
strict <- TreeTools::Consensus(bestTrees, p = 1)
...
By default, these are corrected for chance: 1 denotes unanimous support, 0 no
more agreement than would be expected if the character's tokens were shuffled
across the leaves, and negative values conflict.
...
TreeTools::LabelSplits(strict, signif(concordance, 3),
                       col = TreeTools::SupportColor(pmax(0, concordance)),
                       frame = "none", pos = 3L)
```
...
if (rogueInstalled) {
  ...
  plot(TreeTools::Consensus(noWiwaxia),
...
`MaximizeParsimony()` chooses how hard to search from the dataset itself, since
size and character count predict how much search a matrix repays.
The rungs it chooses between are:

- `sprint` (≤ 30 taxa): fast search with minimal ratchet.
  Good for small or simple datasets.
- `default` (31--64 taxa, or any dataset with < 100 character patterns):
  balanced ratchet and sectorial search.
- `thorough` (≥ 65 taxa with ≥ 100 patterns): intensive search adding
  drift cycles, adaptive ratchet perturbation, more sectorial rounds, and an
  outer cycle loop.
- `large` (≥ 120 taxa): the `thorough` settings with more replicates.

The `effort` parameter shifts that choice up or down.
It is *relative*, not absolute: `effort = 0` (the default) accepts whatever the
dataset warrants, and

```r
MaximizeParsimony(vinther, effort = 1)
```

asks for one notch more than that — so the same call means "try harder than
usual" whether the matrix has 20 taxa or 200, and you never have to know which
rung it started on.
Negative values search less; a large negative value reliably gives the quickest
rung.

Past `large`, further notches double both the replicate budget (1000, 2000,
4000 ...) and the hit target, so one notch always means roughly twice the work.
The rung-4 budget of 500 is measured — on 120–180-tip matrices the fraction of
runs reaching the best score was still climbing at 500 with no knee — but the
doubling above it is an operating point, not a calibrated level: nothing
measures where that curve flattens.
There is no policy ceiling, since extra replicates cost wall but cannot cost
reach.
Replicates are what buy reach; `targetHits` does not.
On 30 inapplicable-bearing matrices, tripling the hit target changed the score
reached on none of them while costing 2.6× the wall — on hard datasets the
replicate cap binds before the hit target is reached, and on easy ones the
optimum is already in hand.
It is raised in step anyway because it governs when *easy* runs stop (without it
a notch would do nothing on any dataset that finishes early), and because under
implied weights it also deepens the ratchet.
So read the higher rungs as buying confidence and distinct trees on easy data,
and reach on hard data.

Any explicit parameter always overrides the rung.
For example, `MaximizeParsimony(vinther, effort = -9, ratchetCycles = 10L)`
searches at the quickest rung but overrides its ratchet setting; a
`maxReplicates` or `targetHits` you supply yourself is never rescaled by
`effort`.
...
```r
MaximizeParsimony(vinther, tree = previousBestTree)
```

Supply a `multiPhylo` and each of its trees seeds a replicate of its own —
replicate _i_ starts from tree _i_ — so a previous search can be resumed from
its whole set of optimal trees rather than from one arbitrary member:

```r
MaximizeParsimony(vinther, tree = previousResult)
```

This matters because the returned trees are topologically diverse, and tree
fusing recombines that diversity from the outset; one tree cannot supply it.
Replicates beyond the supplied pool still start from random Wagner trees.
One tree is consumed per replicate actually run, so a search that converges
early -- on `targetHits`, the usual case -- draws on only part of a large
pool, and warns when it does; raise `targetHits` as well as `maxReplicates`
to use more of it.
...
par(mar = rep(0.25, 4), cex = 0.75) # make plot easier to read
plot(TreeTools::Consensus(iwTrees))
```
