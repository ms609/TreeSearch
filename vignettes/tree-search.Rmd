...
Advanced users might wish to visualize the progress of tree search in mappings
of tree space:

```{r map-search, fig.asp = 1}
distances <- TreeDist::ClusteringInfoDistance(bestTrees)
searchStages <- length(firstHit)
map <- cmdscale(distances, k = 2)
cols <- viridisLite::viridis(searchStages, alpha = 0.8)
par(mar = rep(0, 4))
plot(map,
     col = cols[rep(seq_along(firstHit), firstHit)],
     pch = 16, cex = 2,
     axes = FALSE, xlab = '', ylab = '', frame.plot = FALSE, asp = 1)
...
local optima -- though consciencous users will ensure that the mapping of tree 
...
replicates and search intensity, and instructions for calcuating bootstrap
...
that supports each node, but doesn't necessarily indicate whether the data
...
[@Goloboff2018;@Smith2019].  Better still is to use multiple values and compare the results,
perhaps in 
