...
bestTrees <- Morphy(vinther, tree = bestTrees,
                    ratchIter = 6L,
                    tbrIter = 4L,
                    finalIter = 3L,
                    maxHits = 80L)
...
https://ms609.github.io/TreeSearch/reference/Morphy.html)
...
`MaximizeParsimony()` (which delegates constraint searches to `Morphy()`) will
attempt to find a starting tree that satisfies the constraints, but if it
cannot, it may be necessary to specify one manually
