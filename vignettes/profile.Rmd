...
my.phyDat <- congreveLamsdellMatrices[[10]]
```
...
```{R Starting score}
TreeLength(tree, my.prepdata, 'profile')

better.trees <- MaximizeParsimony(my.prepdata, tree)
```

Let's see the resultant tree, and its score

```{r Ratchet-search-results}
TreeLength(better.trees[[1]], my.prepdata, 'profile')
par(mar = rep(0.25, 4), cex = 0.75) # make plot easier to read
plot(consensus(better.trees))
```
...
suboptimals <- MaximizeParsimony(my.prepdata, better.trees, tolerance = 1.5)
