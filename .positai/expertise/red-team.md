...
reviewed_by: B
date: 2026-03-25
notes: Profile & IW scoring (ts_fitch.cpp 760 lines IW/Profile paths, ts_data.cpp 317 lines precomputation). **Findings:** (1) CONFIRMED CORRECT — `compute_iw()` and `precompute_iw_delta()` handle all edge cases correctly: k=0 guard prevents 0/0 NaN (`if (e == 0) old_cost = 0.0`); k→0 saturation at 1.0; negative e clamped; XPIWE per-pattern `eff_k`/`phi` correctly propagated. (2) CONFIRMED CORRECT — `compute_profile()` and `precompute_profile_delta()` correctly add `precomputed_steps` offset before `info_amounts` lookup, consistent with EW `ew_offset` exclusion for IW/Profile paths in `fitch_score_ew()`. (3) LATENT DEFECT — `precompute_profile_delta()` old_cost boundary: when `idx_old >= info_max_steps`, returns 0.0 instead of max table value. Should match `compute_profile()`'s capping behavior. **Unreachable in practice** because `info_max_steps` covers full range of possible step counts; defensive code handles impossible case incorrectly. (4) CONFIRMED CORRECT — `concavity = 1.0` sentinel (Profile) correctly activates weighted pipeline via `isfinite()` gates in TBR, drift, SPR, ratchet, temper modules. All search modules use `compute_weighted_score()`/`precompute_weighted_delta()` dispatch (never call `compute_iw()` directly). (5) CONFIRMED CORRECT — ratchet `PerturbSnapshot` now saves/restores `active_masks`, `upweight_masks`, and `pattern_freq`. (6) XFORM/HSJ indirect evaluation uses Fitch-only screening (no Sankoff/HSJ DP cost in candidate ranking). By design — full `score_tree()` is authoritative for acceptance. Search quality concern only for hierarchy-heavy datasets. (7) STALE REFERENCES — `test-ts-iw.R` hard-coded IW/EW reference values are invalidated by recent NA handling changes. Large discrepancies on inapplicable-heavy datasets (Wills2012: 627 vs 499 EW, Zhu2013: 3053 vs 2150 EW). Loconte1991 (no NA) shows negligible difference (1 step). References need recomputation. (8) Added 12 new test assertions in `test-ts-iw-profile-red10.R`: ratchet ZERO/MIXED/IW rescore consistency, k=0.01 edge case, Profile driven search rescore, IW+NA rescore for TBR/Ratchet/Drift. All pass.
...
reviewed_by: D
date: 2026-03-25
notes: Fitch scoring correctness — focused on recent XPIWE changes (compute_iw/precompute_iw_delta per-pattern phi/eff_k) and HSJ/XFORM score_tree dispatch. **Findings:** (1) BUG FOUND — `fitch_score_ew()` did not include `ScoringMode::XFORM` in the EW branch condition. XFORM fell to the IW path with `eff_k=0.0` (since concavity=Inf for EW), computing `sum(freq)` for patterns with extra steps instead of actual EW step count. MaximizeParsimony reported wrong scores (e.g. 3 instead of 7 on 6-taxon test). Fix: add `ScoringMode::XFORM` to the EW branch. Filed as T-229, fixed on cpp-search. (2) XPIWE formulas verified against Goloboff 2014 Extension 3: `phi[p] = (1+k/f)/(1+k)`, `eff_k[p] = k/f`. IW score = sum(freq * phi * e/(eff_k+e)). Correct. (3) `precompute_iw_delta` NaN guard for k=0, e=0: correct (returns 0.0 for old_cost instead of 0/0). (4) Minor defensive coding concern: `obs_count == 0` in XPIWE f-factor computation would produce +Inf, clamped by `xpiwe_max_f` check. NaN possible only if `n_tips == 0` AND `obs == 0` (practically impossible). Not filed. (5) IW + hierarchy validation: line 645-648 correctly blocks IW + non-bgs with `stop()`. (6) XFORM SankoffData on-the-fly construction in `score_tree` allocates vectors per call — performance concern for large datasets but not a correctness issue.
EOF 2>&1
