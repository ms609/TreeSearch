...
Gaps represented by the inapplicable token can be treated as "missing
data", i.e. as equivalent to the ambiguous token `?`; as an extra state,
equivalent to other states such as `0` or `1`; or as "inapplicable data"
using the algorithm of Brazeau, Guillerme and Smith (2019).
...
`GapHandler()` returns a character string stating how gaps are handled
by `morphyObj`.
