...
#
# Tests whether reducing base ratchetCycles from 12 to 8/6/4 hurts or
# helps score quality at fixed time budgets. The default preset already
# has adaptiveLevel=TRUE which scales cycles by hit rate, so fewer base
# cycles means more replicates completed in the same wall time.
#
# DESIGNED FOR HAMILTON HPC. Do not run locally.
#
# Usage:
#   Rscript bench_ratchet_cycles.R [timeout_s] [output_dir]
...
cat("=== T-259: Ratchet Cycle Count Experiment ===\n")
...
# ---- Configurations ----
# The default preset has ratchetCycles=12, adaptiveLevel=TRUE.
# We test reducing the base count. Adaptive scaling still applies.
configs <- list(
  ratch_12 = list(
    label = "ratch_12",
    desc = "baseline (default preset, ratchetCycles=12)"
  ),
  ratch_8 = list(
    label = "ratch_8",
    desc = "ratchetCycles=8",
    ratchetCycles = 8L
  ),
  ratch_6 = list(
    label = "ratch_6",
    desc = "ratchetCycles=6",
    ratchetCycles = 6L
  ),
  ratch_4 = list(
    label = "ratch_4",
    desc = "ratchetCycles=4",
    ratchetCycles = 4L
  )
)
...
outfile <- file.path(output_dir,
                     sprintf("t259_ratchet_%ds_%s.csv",
                             timeout_s,
...
# ---- Summary ----
cat("\n=== Summary: Median score and replicates by config x dataset ===\n\n")
for (ds_name in gap_names) {
  ...
  for (cfg_name in names(configs)) {
    ...
    cat(sprintf("    %-12s median=%7.1f (gap %+5.1f)  best=%7.1f  reps=%.0f\n",
                cfg$label, med_score, med_gap, best_score, med_reps))
  }
  cat("\n")
}

# ---- Replicate throughput comparison ----
cat("=== Replicate throughput: median replicates completed ===\n\n")
for (cfg_name in names(configs)) {
  cfg <- configs[[cfg_name]]
  sub <- results[results$config == cfg$label, ]
  med_reps <- median(sub$replicates, na.rm = TRUE)
  mean_gap <- mean(sub$gap, na.rm = TRUE)
  cat(sprintf("  %-12s median_reps=%.0f  mean_gap=%+.1f\n",
              cfg$label, med_reps, mean_gap))
}
