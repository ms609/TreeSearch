...
reps <- list(
  list(
    ...
    new = "Diagnostic signature — CORRECTED 2026-07-29, do not reuse the original form: the claim as filed was \"every phase (TBR, XSS, ratchet) slower on a score-IDENTICAL trajectory ⇒ moves being rejected\". Cumulative per-phase totals do rise (ratchet 1.20x, xss 1.50x, final_tbr 1.24x, arm 2 vs pre-fix), but those totals are summed over however many replicates ran, and a blocked arm runs MORE of them. Normalised PER REPLICATE, arm 2 is CHEAPER in nearly every phase — tbr 0.61x, wagner 0.70x, ratchet 0.83x, fuse 0.45x — which is what rejected moves must do: less work per replicate, not more. Measured from the engine's own `timings` attribute (microsecond resolution, so not a clock artefact), 13 cells x 3 arms. The replicate counts are the whole effect: median `reps_done` 17 pre-fix / **24 = budget exhausted** / 16 merged. **Correct signature: MORE replicates, each CHEAPER, identical score** — a blocked replicate never registers a hit on the best score, so the convergence rule never trips and the search burns its entire budget."
  ),
  list(
