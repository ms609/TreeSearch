...
#   off = TS_IW_NOX4=1   (x4 disabled -> scalar indirect_na_iw_length_cached)
#   on  = TS_IW_NOX4 unset (x4 batch active, default)
#
...
# !has_na-gated so it is INACTIVE here; toggling TS_IW_NOX4 alone isolates the x4
...
run_arm <- function(arm) {
  if (arm == "off") Sys.setenv(TS_IW_NOX4 = "1") else Sys.unsetenv("TS_IW_NOX4")
  set.seed(row$seed)
