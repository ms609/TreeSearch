...
|------|------------|---------------|
| [`focus-areas.md`](focus-areas.md) | The scope table: 15 numbered areas, the files each owns, and its key questions — no tier or status. Built once, edited rarely. **Adding a row** also needs, and nothing currently automates: a matching `area:N` GitHub label (`gh label create area:N --description "Red-team focus area N"`), and recomputing `N` in `log.md`'s rotation-formula comment (see RT12-01). | A round, when it finds the scope row wrong |
| [`log.md`](log.md) | Append-only, **newest first**. One entry per round (`area` / `reviewed_by` / `date` / `tier` / `yield` / `notes`), frozen history; the model-version legend is archived in [discussion #189](https://github.com/agent-issues/TreeSearch/discussions/189). | Every round |
