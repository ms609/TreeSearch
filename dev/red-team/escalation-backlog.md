...
| 5 | **10 — Profile/IW/HSJ/XFORM kernels** | `opus` (Opus 5) | ~~**(a)** the inapplicable IW min-steps convention~~ — **RETIRED 2026-07-28, see Resolved history; do NOT carry it.** **(b)** Un-spent area-10 surface: profile delta capping (`7cff7870`), `e/(k+e)` delta, `precomputed_steps` offset, `info_amounts` capping, `concavity=1.0` profile sentinel, DAT-002 `obs==0` XPIWE division reachability, and the OPEN clipped-subtree IW-screening follow-up. **Plus two items this file did not previously name:** the never-swept `ts_hsj.*`/`ts_sankoff.*` half of the area (zero finder attention at any tier), and the char-ordering (`d8c59998`) × per-pattern IW/profile indexing question. | (b) log.md, area 10, 2026-06-16 (signal-resolution round; finder not spent) | **Weakest dry-evidence in this file.** Both cited dry rounds (2026-05-19, 2026-05-26) are pre-tier and version-unrecorded ⇒ area 10 has **zero** version-scoped dry verdicts. This is a first-versioned measurement, not a re-mine — and area 10 has never had a proper finder sweep. **Dispatched 2026-07-28 at `opus` (Opus 5).** |
