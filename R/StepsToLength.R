#' Calculate Expected Length from Fitch Step Count
#'
#' Calculates the estimated total expected number of evolutionary steps
#' (substitutions) on a phylogenetic tree, given the observed **Fitch parsimony score**.
#' This function applies the **Mk (Markov k-state) model correction**, which is the
#' continuous-time equivalent of the **Jukes-Cantor correction** for discrete,
...
#' \itemize{
#'   \item **Parsimony Gap:** This formula is an approximation. The Fitch score (\eqn{c}) is only a **lower bound** estimate of the true changes, especially for small trees, which causes the output \eqn{N_{actual}} to be underestimated.
#'   \item **Assumptions:** The formula assumes all tree edges are of **equal length** and ignores the tree's **topology**.
#'   \item **Maximum Parsimony Limit:** The formula returns non-finite values (or issues) when the input \eqn{c} approaches the model's theoretical saturation point, \eqn{c_{limit} = e(k-1)/k}. Since the biological maximum Fitch score \eqn{c_{max}} is often lower than \eqn{c_{limit}}, the function may produce very large results when \eqn{c} is near its maximum.
#' }
#'
#' @param steps Numeric. The Fitch parsimony score of a tree (denoted \eqn{c} in the math).
#' @param nTip Integer. The number of leaves (tips) in the tree.
#' @param states Integer. The number of possible tokens (states, denoted \eqn{k} in the math).
#' @return Numeric. The estimated total expected number of steps. Returns \code{NaN} if the input parsimony score is too high to be physically possible under the model.
#' @examples
...
#' @export
StepsToLength <- function(nTip, steps, states) {
  if (!is.numeric(nTip)) {
    nTip <- NTip(nTip)
  }
  ...
  # Term equivalent to (c * k) / (e * (k - 1)) in the denominator of the log.
  # This is the scaled saturation factor: P_obs * (k / (k-1))
  saturation_factor <- (c * k) / (e * (k - 1))
  
  # Check for saturation / mathematical limit (ln(0) = -Inf, but here we expect positive result)
  # The argument to the logarithm must be > 0.
  if (any(saturation_factor[!is.nan(saturation_factor)] >= 1)) {
    warning("Observed parsimony score is at or above the model's theoretical saturation limit.")
  }
  
  # Calculate N_actual: -(e * (k - 1) / k) * log(1 - saturation_factor)
  
  ifelse(saturation_factor >= 1 | is.nan(saturation_factor), NaN,
         -(e * (k - 1) / k) * log(1 - saturation_factor))
}
