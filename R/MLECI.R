...
MLCI <- function(
  ...
  # best split (same as your version)
  splits <- TreeTools::as.Splits(tree)
  im <- TreeTools::SplitImbalance(splits)
  best <- splits[[which.min(im)]]
  best_states <- as.integer(as.logical(best))
  
  res <- data.frame(
    t_hat = numeric(nChars),
    score = numeric(nChars),
    rmMean = numeric(nChars),
    rmSE = numeric(nChars),
    bestScore = numeric(nChars),
    mlci = numeric(nChars),
    nResample = integer(nChars)
  )
  ...
  for (j in seq_len(nChars)) {
    ...
    tip_states_obs <- unname(map[col])
    
    out <- mlci_resample(
      edge, Ntip, tip_states_obs, best_states,
      precision, maxResample,
      lower, upper, tol
    )
