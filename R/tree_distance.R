...
#' @author Martin R. Smith
#' @export
VariationOfInfo <- function (tree1, tree2, reportMatching = FALSE) {
  if (class(tree1) == 'phylo') {
    if (class(tree2) == 'phylo') {
      if (length(setdiff(tree1$tip.label, tree2$tip.label)) > 0) {
        stop("Tree tips must bear identical labels")
      ...
      VariationOfSplitInfo(Tree2Splits(tree1), Tree2Splits(tree2), reportMatching)
    } else {
      splits1 <- Tree2Splits(tree1)
      vapply(tree2, 
             function (tr2) VariationOfSplitInfo(splits1, Tree2Splits(tr2)),
             double(1))
    }
  } else {
    if (class(tree2) == 'phylo') {
      ...
      splits1 <- lapply(tree1, Tree2Splits)
      splits2 <- lapply(tree2, Tree2Splits)
      matrix(mapply(VariationOfSplitInfo, rep(splits1, each=length(splits2)), splits2),
             length(splits2), length(splits1), dimnames = list(names(tree2), names(tree1)))
    }
  }
}
...
#' @export
VariationOfSplitInfo <- function (splits1, splits2, reportMatching = FALSE) {
